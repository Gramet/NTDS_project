...
def predictors(df, feature, model, modelname, displayResults = True, displayColors = False):
    ...
    model, acc = model_test(model,X_train,y_train,X_test,y_test, full_voc, displayResults = displayResults, displayColors = displayColors)
    
    return model, full_voc, acc
	
def test_external_data(text, full_voc, model):
...
def combine_features(model_text, model_pic, model_color, data, voc_text, voc_pic, voc_color, acc_text, acc_pic, acc_color):
    ...
    for i in range(0,len(data)):
        ...
        proba_color, class_color = test_external_data(data['link_color'][i:i+1], voc_color, model_color)
